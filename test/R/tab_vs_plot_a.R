...
## creation date: 2020-09-09 16:04:51
## Author: 
...
ns('vd_data'))
       ...
       title = "You need to prepare Raw data from these tabs:"
        ...
        ##### select input data
        fluidRow(
            column(6, shinyWidgets::pickerInput(ns("source_data"), "Raw data",
choices = c("Data Tab Title" = "data_a"),
...
plot_aServer <- function(id, shared){
...
pgPaneUpdate("pg", "vd_data", 100)
        ...
        observeEvent(input$render, {
            ...
            output$plot <- sps_plots$addServer(plotly::renderPlotly, tab_id, {
                plotly::ggplotly(ggplot(mydata$data, aes_string(names(mydata$data)[1], names(mydata$data)[2])) + geom_point(
  aes(color = seq_len(nrow(mydata$data)))))
            })
            #####
