...
```{r spsinit, eval=TRUE}
sps_tmp_dir <- tempdir()
spsInit(dir_path = sps_tmp_dir, change_wd = FALSE, project_name = "SPSProject")
...
On default start, current SPS contains the workflow tabs and one example data tab, and 
...
SPS tabs, data, and other helper functions. Using `spsLoadPlugin()` function, you 
are able to load them into SPS framework. 

Using `spsLoadPlugin()` without any argument will list all available plugins and 
...
```{r , eval=FALSE}
spsLoadPlugin(plugin = "PLUGIN_NAME", app_path = "PATH_OF_SPS")
```
...
will help you to create the tab file to the *R* folder in your SPS project **and** 
...
- Augments with `xx_func` or `xx_expr` use [non-standard evaluation](http://adv-r.had.co.nz/Computing-on-the-language.html), please do not add quotes. If you want to use quotes, set `use_string = TRUE`
...
specify one unique input data set and source data tab options. For example, we want to 
...
tabular data files with default templates). You can create your own 
...
so each tab is like a small sandbox and variables, objects do not conflict with other tabs. 
...
regular expression match. By default, if it matches more than one ID, functions aborts,
...
if you want to do it in a programmatic way or use it in a loop. Once the function 
...
Only the *Plygin_root/inst/app/config/tabs.csv*. So one can easily move a SPS project 
to a SPS plugin by copying all files into *Plygin_root/inst/app/*.

If your plugin provides some SPS tabs, you can use `newTabData(plugin = "PLUGIN_NAME", ...)` 
...
In additional to tab files, all other files in *Plygin_root/inst/app/* will also be copied to 
users' SPS project. 

At this point, this is just a very basic structure of a SPS plugin. To make it more publishable 
like a normal R package, it is recommended to use `usethis::create_package("PLUGIN_ROOT_PATH")` and
other functions come with `usethis`to add additional files like *man*, *vignettes* that 
are required by a formal R package. Please contact us if you want to have your plugin to 
be "officially", otherwise, you can use `spsLoadPlugin(..., third_party = TRUE)` to load 
...
SQLite database which on current version stores some a table of meta information of 
