...
`c("tab1", "tab2)`. See *config/tabs.csv* or use `spsTabInfo()` what tabs IDs can be load and other 
tab information. Currently you can only load/unload visualization tabs, type of `vs`; essential framework tabs(*core*) and workflow tabs (*wf*) are loaded automatically and these tabs cannot be modified. 

Also, SPS tabs are built on top of [Shiny modules](https://shiny.rstudio.com/articles/modules.html) and there is one top-level server that is above all modules. You can run additional top-level server code by passing it to `server_expr` argument. If the expression is multiple-lined, use `{}` to wrap them. 
...
be "official", otherwise, you can use `spsLoadPlugin(..., third_party = TRUE)` to load 
