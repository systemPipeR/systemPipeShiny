...
(SPS)^[<font style="color:red">ThG: the text that was here before wasn't really 
an introduction, nor did it describe the purpose of this package. Thus, I have 
changed it mostly to the text of your recent poster. Make sure what is described
here matches mostly what the package does or should do.</font>] extends the widely 
used [systemPipeR](https://bioconductor.org/packages/systemPipeR/) (SPR) workflow
...
plots, data mining plots including heatmaps, and more. The modular design of
...
visualization (VS) toolbox (*Fig 1*).^[<font style="color:red">ThG: I have changed
this section significantly.</font>] The WF module allows to write and to
modify configuration files required for running workflows in `systemPipeR`. The
VS part provides a rich set of plotting tools to visualize the results of
individual steps in `systemPipeR` workflow. The latter includes a Canvas
functionality that allows to arrange many plots one page, save their configuration
and revisit the saved page at a later time.
...
and stored in individual files. This modular structure is highly extensible 
...
The following sections illustrate the main functionalities SPS using an analysis 
workflow for RNA-Seq data as an example.

## Load package

Load the `systemPipeShiny` package in your R session.^[<font style="color:red">ThG: 
note the package load was missing.</font>]
...
```{r , eval=FALSE}
spsInit(project_name = "SPSProject")
sps_dir <- "SPSProject"
```

```{r spsinit, eval=TRUE, echo=FALSE}
sps_tmp_dir <- tempdir()
...
Before launching the SPS application, a project environment needs is created with the
following command:

```{r spsinit, eval=FALSE}
spsInit()
```
>>>>>>> 0b1d43cf2d4d53733ee53af2745d3fc3d331f419

By default, `spsInit` creates a project folder under the current working directory
of a user's R session. The name of this directory is `SPS_` + `today's date` 
in *ISO 8601* format, e.g. `r glue::glue("SPS_{format(Sys.time(), '%Y%m%d')}")`. 
This project environment is populated with all the required files to run the application. 

It is possible to choose a different location, as well build the environment in a temporary directory. By default, the function set the working directory to the project folder, and it can 
turn off with the function option, as follows:

```{r spsinit_details, eval=TRUE}
spsInit(app_path = tempdir(), change_wd = FALSE, project_name = "SPSProject")
sps_dir <- file.path(tempdir(), "SPSProject")
```

Note that in this example, the project directory structure is written to a temporary directory
on a user's system. For a real project, it should be written to a defined and user 
controlled location on a system rather than a temporary directory. 

### Directory structure

The file and directory structure is organized as follows:
...
## Launch SPS interface

The `runApp` function launches the application in our browser:

```{r runAPP, eval=FALSE}
shiny::runApp(app)
```
^[<font style="color:green">Dani: Maybe add a printscreen here with the main page of the pp.</font>]
...
`shiny::runApp()`^[<font style="color:red">ThG: This is the most important command of 
the entire vignette. Why is it burried in the text. I would move it up to the beginning and
dedicate a code box to it.</font>] in console or click on the <img src=img/app_start.png style="display: inline; padding-left:0; width:10px; height:15px;">`Run App`
...
Workflow^[<font style="color:red">ThG: I wouldn't abbreviate work throught the text.</font>] management in SPS helps you to prepare a data analysis workflow files in [systemPipeR](https://systempipe.org/docs/systemPipeR/) (SPR) format. SPR is another 
...
structure of this file. ^[<font style="color:red">ThG: describe here briefly what a targets
file is and how it relates to the colData slot in an SummarizedExperiment object.</font>]
...
|-----------------|-----------------------------------|---------|-----------|
| mode            | running mode                      | "server"| "local"   |
