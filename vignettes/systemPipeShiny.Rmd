...
The targets file describe all input samples for the analysis workflow, defining all the input file path information, sample name and comparisons, and any additional metadata associated with the sample. More details can be found on ["Structure of targets
file"](https://systempipe.org/docs/systemPipeR/#structure-of-targets-file)
section in `systemPipeR` vignette.

^[<font style="color:green">Dani: Should we load the targets file here to show an example on the vignette?</font>]
