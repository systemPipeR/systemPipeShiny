...
plots, data mining plots including heatmaps, and more. 
^[<font style="color:green">Dani:My opinion here is that if we mentioned some plot here, 
we should provide the plots on the visualization tabs since we already have these plots organized.</font>]
...
**Figure 1. Design of `systemPipeShiny.` The framework provides an interactive web interface for workflow management and data visualization.**
</center>
...
For this toy example, the project directory structure is written to a temporary directory
...
The file and directory structure is organized as follows:^[<font style="color:green">Dani:Should we have a dedicated section to the directory structure?.</font>]
...
Then, to launch the `SPS` application, run:

```{r, eval=FALSE}
shiny::runApp()
```

Or you can click on the <img src=img/app_start.png style="display: inline; padding-left:0; width:10px; height:15px;">`Run App`
button on top right corner in Rstudio.

In addition, if you are using Rstudio, *global.R* file will be automatically open when 
`SPS` project is created. Custom changes can be made inside this file before the app launches. The [advanced section](#extend-sps) will teach you how to create your own tabs.
^[<font style="color:green">Dani:I think this advanced information could be confusing in this stage of the documentation (discussing and showing details about the global.R file), so I believe we can add this to the advanced features section and just to link here.</font>]

For this vignette example, we will use as it is, so no change is made.
The app should be launched at this point. Click on the "Continue to app" button 
on the welcome screen and you should see the `SPS` main dashboard like this one (Fig.2):
...
All `SPS` tabs are well classified into Workflow and Visualization modules. 

## Workflow management

Workflow management in `SPS` helps you to prepare a data analysis workflow files in [systemPipeR](https://systempipe.org/docs/systemPipeR/) (SPR) format. SPR is another 
R package that runs a workflow in "command line + R code" manner. To run a SPR workflow, you
need to prepare three very important files: a metadata (targets) file that contains 
sample information, a workflow step file (Rmarkdown format) that defines the workflow steps 
and a workflow config file that helps the program to set up your directory. In SPS, one 
...
files. One should save these files and run a workflow in SPR. Directly running a SPR workflow in SPS will be supported in an upcoming release.

### Prepare a target file

The targets file defines all input files' path and other sample information of 
an analysis workflow. Read ["Structure of targets file"](https://systempipe.org/docs/systemPipeR/#structure-of-targets-file) 
section in SPR vignette to better undertand the structure of this file. 
This file is similar to the `colData` slot in an `SummarizedExperiment` object 
which stores sample ID and other meta information. 


Now let us follow the order below to prepare a targets file for a RNAseq project (Fig.3 A):
...
4. SPR target file requires a header and you can edit it here. Headers should start with "#";
...
5. Click on the "Plot steps" will show a flow chart of what the step execution orders will be when you run the workflow in SPR. 
6. Click "Report preview" generates a preview of what the final report will look like for current RNAseq workflow, but in the preview, no code is evaluated.
