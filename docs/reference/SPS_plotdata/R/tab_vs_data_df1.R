###################### SPS df 1 tab######################
## creation date: 2020-09-29 14:18:45
## Author:

# data_df1 UI
data_df1UI <- function(id) {
    ns <- NS(id)
...
data_df1Server <- function(id, shared) {
    module <- function(input, output, session) {
        ...
        data_df <- reactive({
            ...
            loadDF(
                ...
                eg_path = '/home/dcassol/danielac@ucr.edu/testing/systemPipeShiny/docs/reference/SPS_plotdata/data/iris.csv'
            )
        })
        # display table
        ...
        observeEvent(input$preprocess, ignoreInit = TRUE, {
            ...
            switch(
                input$select_prepro,
                'nothing' = spsValidate(is.data.frame(data_filtered)),
                'md1' = {
                    nrow(data_filtered) > 1
                }
            )
