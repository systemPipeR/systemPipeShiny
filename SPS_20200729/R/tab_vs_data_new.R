...
## creation date: 2020-07-29 01:46:02
## Author:

# data_new UI
data_newUI <- function(id) {
    ns <- NS(id)
    ...
    tagList(
        ...
        tabTitle("Title for my first data tab"), spsHr(),
...
data_newServer <- function(id, shared) {
    module <- function(input, output, session) {
        ...
        data_df <- reactive({
            ...
            loadDF(
                ...
                eg_path = '/home/lz/Desktop/systemPipeShiny/SPS_20200729/data/iris.csv'
            )
        })
        # display table
        ...
        observeEvent(input$preprocess, ignoreInit = TRUE, {
            ...
            switch(
                input$select_prepro,
                'md1' = spsValidate(is.data.frame(data_filtered))
            )
